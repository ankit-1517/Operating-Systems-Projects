...
echo "CASE4: Reader Preference with 5 reader and 10 writer"
total=$((total+1))
out=`./rwlock-reader-pref 5 10`
echo $out
if [ "$out" = "PASSED" ]; then
    correct=$((correct+1))
fi

echo "CASE5: Reader Preference with 0 reader and 1 writer"
total=$((total+1))
out=`./rwlock-reader-pref 5 5`
echo $out
if [ "$out" = "PASSED" ]; then
...
echo "CASE4: Writer Preference with 5 reader and 10 writer"
total=$((total+1))
out=`./rwlock-writer-pref 5 10`
echo $out
if [ "$out" = "PASSED" ]; then
    correct=$((correct+1))
fi

echo "CASE5: Writer Preference with 1 reader and 0 writer"
total=$((total+1))
out=`./rwlock-writer-pref 1 0`
echo $out
if [ "$out" = "PASSED" ]; then
    correct=$((correct+1))
fi
