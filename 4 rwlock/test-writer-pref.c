...
int main(int argc, char *argv[])
{
	...
	//Min max from above
	long max_reader_acquire_time_first_half = LONG_MIN;
	long min_reader_release_time_first_half = LONG_MAX;
	long max_reader_release_time_first_half = LONG_MIN;
	long min_reader_acquire_time_second_half = LONG_MAX;
	long max_reader_acquire_time_second_half = LONG_MIN;
	long min_reader_release_time_second_half = LONG_MAX;
	long min_writer_acquire_time = LONG_MAX;
	long max_writer_release_time = LONG_MIN;
	for (int i=0; i<read_num_threads; i++){
		if (readerAcquireTime[i] > max_reader_acquire_time_first_half){
			max_reader_acquire_time_first_half = readerAcquireTime[i];
		}
	}
	for (int i=0; i<read_num_threads; i++){
		if (readerReleaseTime[i] < min_reader_release_time_first_half){
			min_reader_release_time_first_half = readerReleaseTime[i];
		}
		if (readerReleaseTime[i] > max_reader_release_time_first_half){
			max_reader_release_time_first_half = readerReleaseTime[i];
		}
	}
	for (int i=read_num_threads; i<2*read_num_threads; i++){
		if (readerAcquireTime[i] < min_reader_acquire_time_second_half){
			min_reader_acquire_time_second_half = readerAcquireTime[i];
		}
		if (readerAcquireTime[i] > max_reader_acquire_time_second_half){
			max_reader_acquire_time_second_half = readerAcquireTime[i];
		}
	}
	for (int i=read_num_threads; i<2*read_num_threads; i++){
		if (readerReleaseTime[i] < min_reader_release_time_second_half){
			min_reader_release_time_second_half = readerReleaseTime[i];
		}
	}
	for (int i=0; i<write_num_threads; i++){
		if (writerAcquireTime[i] < min_writer_acquire_time){
			min_writer_acquire_time = writerAcquireTime[i];
		}
		if (writerReleaseTime[i] > max_writer_release_time){
			max_writer_release_time = writerReleaseTime[i];
		}
	}
