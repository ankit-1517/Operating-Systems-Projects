...
int main(int argc, char *argv[])
{
	...
	// Find min max read write acq rel time
	long max_reader_acquire_time = LONG_MIN;
	long min_reader_release_time = LONG_MAX;
	long max_reader_release_time = LONG_MIN;
	long min_writer_acquire_time = LONG_MAX;
	for (int i=0; i<2*read_num_threads; i++){
		if (readerAcquireTime[i] > max_reader_acquire_time){
			max_reader_acquire_time = readerAcquireTime[i];
		}
	}
	for (int i=0; i<2*read_num_threads; i++){
		if (readerReleaseTime[i] < min_reader_release_time){
			min_reader_release_time = readerReleaseTime[i];
		}
		if (readerReleaseTime[i] > max_reader_release_time){
			max_reader_release_time = readerReleaseTime[i];
		}
	}
	for (int i=0; i<write_num_threads; i++){
